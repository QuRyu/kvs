...
use crate::{KvsEngine, Result, Request, Response};
    ...
    fn serve(&mut self, stream: TcpStream) -> Result<()> { 
        ...
        macro_rules! send_resp {
            ( $op: expr, $resp: expr ) => {
                ...
                writer.flush()?;
                debug!(self.logger, "{} response sent to {}: {:?}", $op, peer_addr, rep);
            };
        ...
        for req in reader { 
            match req? {
                Request::Set { key, value } => {
                    ...
                    writer.flush()?;
                    debug!(self.logger, "Set response sent to {}: {:?}", peer_addr, resp);
                }

                Request::Get { key } => { 
                    send_resp!("Get", { match self.engine.get(key) { 
                        Ok(v) => Response::Ok(v), 
                        Err(e) => Response::Err(e.to_string()),
                    }});
                }
                ...
                Request::Remove { key } => {
                    send_resp!("Remove", { match self.engine.remove(key) { 
                        Ok(_) => Response::Ok(None),
                        Err(e) => Response::Err(e.to_string()),
                    }});
